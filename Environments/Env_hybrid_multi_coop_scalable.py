...
class pedestrian:
    ...
    def __init__(self, ped_b, car_b, cross, lines, limit_speed, dt,
        ...
        if(not self.exist):
            ...
            self.initial_pos[1] = ped_b[0,3]* self.direction
    ...
    def choix_pedestrian(self, cars_speed, cars_pos, cars_line, cars_light, car_size=4):
        car_time=0
        #print("Name pd ",self.number_name)
        #print("CG is ",CG)
        #for i in range(len(cars_speed)):
        #    if(self.is_in_front(cars_line[i],next_line=1.0)):
        #        print("car_time is ",abs((cars_pos[i]-self.Sp_x) / (cars_speed[i]+10e-3)))
        #        print("car speed ",cars_speed[i])
        #        print("car pos ",cars_pos[i])
        #print("ped pos ",self.Sp_x)
        #pos_line = (self.Sp_y + self.cross_lines/2)//self.cross
        if(self.follow_rule):
        ...
        for i in range(len(cars_speed)):
            if(self.is_in_front(cars_line[i],next_line=1.0)): # in front qnd not in the same line if you cross more than half the line (always the case in the middle)
                if (cars_pos[i] < car_size+self.Sp_x) * (cars_pos[i] > self.Sp_x):
                    return False
    ...
    def detection(self, cars,prev_cars_pos):
        ...
        for i in range(len(prev_cars_pos)):
            if(self.is_in_front(cars[i].line) and cars[i].exist):
                ...
                #ped_in_danger2=(not self.possible_accident)*(self.worst_scenario_accident)
                #if(self.is_crossing_in_front(cars[i].line)*(cars[i].Sc<self.Sp_x)):
                #    cars[i].Ts=max(self.Sp_x-cars[i].Sc,cars[i].Ts)
                #cars_light_green=sum([1.0 for car in cars if car.light>0.])
                cars_light_waiting=sum([1.0 for car in cars if car.light>0. and car.Sc<self.Sp_x and car.exist])
                ...
                #if ped_in_danger2*(self.ped_in_cross or self.at_crossing)*(cars[i].light<0.0)*(cars[i].Sc<self.Sp_x):
                    #self.is_crossing_in_front(cars[i].line,prev_line=0.01)
                    #self.ped_in_cross): #self.is_crossing_in_front(cars[i].line,prev_line=0.01)
                    #cars[i].possible_accident2 = 1. # test avec Ts , waiting marche bien aussi
                    #print("Possible refus de priorite : ", self.ped_left)
        ...
        #for i in range(len(prev_cars_pos)):
        ##    cars_light_green=sum([1.0 for car in cars if car.light>0. and car.Ts>0])
        #    if(cars[i].light<0.0)*(cars[i].Ts>0)*self.is_in_front(cars[i].line): #*(cars_light_green>0.)
        #        car_Ts.append(-cars[i].Ts*(cars_light_red))
        #    else:
        #        car_Ts.append(0.)
        cars_light_green=sum([1.0 for car in cars if car.light>0. and car.exist])#and car.exist
        cars_possible_accident=sum([car.possible_accident for car in cars if (cars[i].light<0.0 and car.exist)])
        cars_possible_accident2=sum([car.possible_accident2 for car in cars if car.exist])
        #cars_Ts=sum([-car.Ts for car in cars if car.light<0. and car.Ts>0 and car.line!=cars[i].line])
        for i in range(len(prev_cars_pos)):
            #cars_error_scenario=sum([car.error_scenario for car in cars if car.line!=cars[i].line])
            ...
            #if(cars[i].light<0.) and (cars_light_green>0):
            
            #	cars_error_scenario=min([car.error_scenario for car in cars if car.light<0.])
            #	res_consistent += cars_error_scenario#*(cars_error_scenario<-1.) #cars[i].error_scenario # (- 1.*cars[i].error_scenario)
            res = res_accident+res_consistent-0.5*cars_light_green*(cars[i].light<0.)*(cars[i].Ts>0) # derniere partie inutile
            #on evite l accident grave, on evite les erreurs de signalisation (green plus grave), enfin on evite les risques de collision mineures ou de refus de priorite
            if (cars[i].exist):
                car_dangers.append(res)
            else:
                car_dangers.append(0.)
    ...
    def step(self, time, cars_speed, cars_pos, cars_line, car_light):
        ...
        if(self.Sp_y*self.direction <= (-self.cross_lines/2)):
            ...
            #if((-self.cross_lines/2--self.cross/2)<self.Sp_y*self.direction):
        ...
        if(self.is_crossing):
            ...
            # Pedestrian in crosswalk
                ...
                #cars_speed_l=[cars_speed[i] for i in range(len(cars_line)) if self.is_in_front(cars_line[i])]
                #cars_pos_l=[cars_pos[i] for i in range(len(cars_line)) if self.is_in_front(cars_line[i])]
                ...
                #elif (self.change_line) and not choix_pedestrian(cars_speed_l, cars_pos_l):
                ...
                # The pedestrian walks
                    ...
                    if(self.stop):
                        ...
                        self.stop=True
                        #distance = abs(self.direction*self.line_pos*self.cross - self.direction*self.cross_lines/2. -self.Sp_y)
    ...
    def get_data(self, cars_pos, cars_speed, cars_line, cars_light):
        ...
        #delta_pos_l=[(self.Sp_x-car_pos) for car_pos in cars_pos if (self.Sp_x-car_pos)>0]
        #if(len(delta_pos_l)):
        #    delta_pos=min(delta_pos_l)
        #else:
        #    delta_pos=0.
        self.delta=min(self.delta_l_all(cars_pos, cars_speed, cars_line, cars_light)* (self.is_crossing)* ( not self.ped_left),self.delta)
        #random_all=[random.uniform(-0.05, 0.05) for i in range(4)]
        #return [self.Vp_x+random_all[0], self.Vp_y+random_all[1], self.Sp_x+random_all[2], self.Sp_y+random_all[3], self.delta, self.ped_left, self.ped_in_cross, self.exist] 
    ...
    def new_reward_wait_safety(self, car_speed, car_pos, car_line):
        ...
        if ((not self.ped_left) * (self.is_crossing) * (car_pos < self.Sp_x) * self.is_in_front(car_line)):
            if(car_speed<0.05):
                ...
                #elif(car_speed/self.limit_speed<0.1):
                #    exp_dl=-20.
                else:
                    exp_dl=20.*dl
            exp_dl=exp_dl -(self.accident)*20
            if(exp_dl<self.worst_dl):
                #print("Car pos is ", car_pos)
                #print("Car speed is ", car_speed)
                #print(exp_dl)
                self.worst_dl=exp_dl
        #if(not self.exist or not self.is_crossing):
        #    return 0.0
        # Safety_reward
        rew1=self.worst_dl
        #if(self.error_scenario or self.possible_accident):
        #    rew1 = self.worst_dl#-20.
        # Others rewards?
        rew2 = 0.0
        return rew1 + rew2
...
class car:
    ...
    def __init__(self, car_b, ped_b, cross, lines, dt, speed_limit, i, exist=True):#, speed_ped_list, pos_ped_list):
        #self.seed(seed)
        self.car_b = car_b
        self.dt=dt
        self.cross=cross
        ...
        self.Ts=0.
        # Initial car position
        #if(len(speed_ped_list)>0 and len(pos_ped_list)>0):
        #    mean_speed_ped=sum(speed_ped_list)/len(speed_ped_list)
        #    min_speed_ped=min(speed_ped_list)
        #    max_speed_ped=max(speed_ped_list)
        #    min_pos_ped=min(pos_ped_list)
        #    max_pos_ped=max(pos_ped_list)
        #else:
        # Initial car position
        ...
        #ped_pos_x=(sum(pos_ped_list)/len(pos_ped_list)) 
        ...
        self.initial_pos = random.uniform(low_car_range - finish_crosslines_time, high_car_range )-20.0*(i%2)#- 0.5*mid_cross_time)
    ...
    def sigma(self, a, e=1):
        ...
        if(a>0):
            ...
            #sg = (math.tanh((a/self.Vc)+e)+1)/2 # limitation of the acceleration range if speed is 
            #if(sg*a<-self.Vc/(self.dt)):
            #    sg = 0
            return sg
    ...
    def step(self, action_acc, action_light, obs_leader=None):
        action_idm=2.
        if(obs_leader):
            action_idm=self.follow_action(obs_leader)
        action=min(action_idm,action_acc)
        self.previous_acc.popleft()
        acc=self.acceleration(action)
        #acc = min(acc,(self.initial_speed*2.-self.Vc)/self.dt)
        sg=self.sigma(acc)
        #if(sg>0.):
        #    acc = max(acc,-self.Vc/(self.dt*sg))
    ...
    def get_data(self):
        if(not self.exist):
            return [0., 0., 10., -1000., 0, self.line, 0]
        return [self.Ac, self.Vc, self.initial_speed-self.Vc, self.Sc, self.light, self.line, self.exist]
...
class Crosswalk_hybrid_multi_coop_scalable(gym.Env):
    """Custom Environment that follows gym interface"""
    metadata = {'render.modes': ['human']}
    """
        Crosswalk_hybrid_multi_coop_scalable: creation of the environment
        :param dt: step time
    ...
    def __init__(self, car_b, ped_b, cross_b, nb_car, nb_ped, nb_lines, dt, max_episode, simulation="unif"):
        super(Crosswalk_hybrid_multi_coop_scalable, self).__init__()
        #self.seed()
        self.viewer = False
        self.window = None
        ...
        self.action_space = Box(low = np.repeat(np.array([-1.0,-1.0]),2*nb_lines),
                                high = np.repeat(np.array([1.0,1.0]),2*nb_lines),
                                shape = (2*2*nb_lines,), dtype=np.float32) 
        # respectively car acceleration (0), car_speed(1), car speed diff (2), car position (3), light(4), line (5), exist(6)
        car_low = np.array([self.car_b[0,0], np.finfo(np.float32).min, np.finfo(np.float32).min,
                            np.finfo(np.float32).min,-1,0,0])
        car_high = np.array([self.car_b[1,0], np.finfo(np.float32).max, np.finfo(np.float32).max,
                             np.finfo(np.float32).max,1,nb_lines,1])
        ...
        # respectively sizealllines (0), nb_ped (1), nb_car, lines (1-4 par exemple)
        #any pedestrian in crosswalk , all pedestrian leave crosswalk, 
        env_low = np.array([ 0.1, 0, 0, 0])
        env_high = np.array([ self.cross_b[1]*nb_lines, self.nb_ped, 2*nb_lines, nb_lines])
        # we could add pedestrian presence in crosswalk and pedestrian finish crossing
        size_cars=2*self.nb_lines
        self.observation_space = Dict(
            {"car": Box(low=np.repeat(car_low,size_cars,axis=0).flatten(),#TODO: SEQUENCE FOR CAR RANDOM NUMBER
                        high=np.repeat(car_high,size_cars,axis=0).flatten(),
                        shape=(size_cars*7,), dtype=np.float32),
    ...
    ##    possible_accident=0
     ...
     #       possible_accident+=ped.possible_accident
     #       error_scenario+=ped.error_scenario
     #   reward= - possible_accident - error_scenario
     #   return reward
    ...
    def step(self, actions):
        ...
        for i in range(self.nb_lines*2):
            if(i%2==1 and self.cars[i-1].exist and self.cars[i].exist):
                self.cars[i].step(actions[i],actions[i+self.nb_lines*2],self.cars[i-1].get_data())
            else:
                self.cars[i].step(actions[i],actions[i+self.nb_lines*2])
        cars_Sc = [car.Sc for car in self.cars if car.exist]
        cars_line = [car.line for car in self.cars if car.exist]
        cars_Vc = [car.Vc for car in self.cars if car.exist]
        cars_light = [car.light for car in self.cars if car.exist]
        cars_time_braking = [car.time_braking for car in self.cars if car.exist]
        for ped in self.pedestrian:
        ...
        #car_acc, car_speed, car_pos, car_speed_diff = self.car.get_data()
        car_dangers=np.array([0.]*(2*self.nb_lines))
        for ped in self.pedestrian:
            detection=ped.detection([car for car in self.cars],prev_cars_Sc)
            if ped.is_crossing and len(detection) and ped.exist:
        ...
        #reward1 = [car.new_reward_cross() for car in self.cars if (car.light<=0.5)]
        #reward2 = sum([car.new_reward_wait_speed() for car in self.cars if (car.light>0.5)])
        #cars_Vc_2 = [car.Vc for car in self.cars if (car.light>0.5)]
        #cars_Sc_2 = [car.Sc for car in self.cars if (car.light>0.5)]
        #cars_line_2 = [car.line for car in self.cars if (car.light>0.5)]
        #reward2 += min([ped.new_reward_wait_safety(cars_Vc_2, cars_Sc_2, cars_line_2) for ped in self.pedestrian])
        ...
        self.state["env"] = np.float32(np.array([self.cross*self.nb_lines/2., self.ped_traffic, self.car_traffic, self.nb_lines]))
    ...
    def reset(self, seed=None, options=None):
        ...
        self.cars = [car(self.car_b, self.ped_b, self.cross, self.nb_lines, self.dt,
                         self.speed_limit, i//2, exist=False) for i in range(2*self.nb_lines)]
        self.car_traffic=random.randint(1,self.nb_car)
        cars_numbers=random.sample([i for i in range(2*self.nb_lines)],self.car_traffic) #choix des vehicules/voies
        for i in cars_numbers:
            self.cars[i]=car(self.car_b, self.ped_b, self.cross, self.nb_lines, self.dt,
                         self.speed_limit, i//2, exist=True)#random.randint(0,self.nb_lines-1)
