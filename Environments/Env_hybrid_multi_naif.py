...
class pedestrian:
    ...
    def __init__(self, ped_b, car_b, cross, lines, limit_speed, dt,
        ...
        self.follow_rule = random.randint(0,9)<10 #100% de chance que le pieton suive les indications du vehicule (help converging)
        self.exist=exist
    ...
    def reset_ped(self, speed_x, speed_y, pos_x, pos_y, dl, direction, cross):
        self.cross=cross
        self.cross_lines=self.max_lines*self.cross
        self.direction = direction
        self.initial_speed = [speed_x,speed_y*self.direction]
        self.ratio=self.initial_speed[0]/self.initial_speed[1]
        self.initial_pos =[ pos_x, (pos_y-self.cross_lines/2.)*self.direction]
        self.Vp_x, self.Vp_y = self.initial_speed
        self.Sp_x, self.Sp_y = self.initial_pos
    ...
    def choix_pedestrian(self, cars_speed, cars_pos, cars_line, cars_light, car_size=4):
        ...
        if(self.follow_rule):
            ...
            for i in cars: # test if cars in crosswalk in the first lane
                ...
                if(self.is_crossing_in_front(cars_line[i],prev_line=0.5)*(self.is_in_front(cars_line[i],next_line=1.0))*(cars_pos[i] < car_size+self.Sp_x) * (cars_pos[i] > self.Sp_x)):
                    return False
    ...
    def detection(self, cars,prev_cars_pos):
        ...
        for i in range(len(prev_cars_pos)):
            if(self.is_in_front(cars[i].line)):
                ...
                #self.danger_scenario_accident = 1 if self.delta_l(cars[i].Sc, cars[i].Vc,cars[i].line)<0 else 0
                #ped_in_danger=(not cars[i].possible_accident)*(self.worst_scenario_accident)
                #ped_in_danger2=(not cars[i].possible_accident2)
                ...
                if(cars[i].Sc<self.Sp_x):
                    cars[i].Ts=max(self.waiting_time+10.*self.crossing_time-cars[i].time_braking+1., cars[i].Ts)
        ...
        for i in range(len(prev_cars_pos)):
            cars_error_scenario=sum([car.error_scenario for car in cars if car.light<0. and car.Ts>0 and car.line!=cars[i].line])
            res_accident = cars[i].possible_accident #-cars[i].Ts*(cars[i].light<0.0)# (self.waiting_time-0.5)
            res_consistent = cars[i].error_scenario #cars[i].error_scenario # (- 1.*cars[i].error_scenario)
            res = res_accident+res_consistent#+cars_error_scenario*(cars_light_green>0)*(cars[i].light<0.)*(cars[i].Ts>0)
...
class Crosswalk_hybrid_multi_naif(gym.Env):
    """Custom Environment that follows gym interface"""
    metadata = {'render.modes': ['human']}
    """
        Crosswalk_hybrid_multi_naif: creation of the environment
        :param dt: step time
    ...
    def __init__(self, car_b, ped_b, cross_b, nb_car, nb_ped, nb_lines, dt, max_episode, simulation="unif"):
        super(Crosswalk_hybrid_multi_naif, self).__init__()
        #self.seed()
        self.viewer = False
        self.window = None
    ...
    def reset(self, seed=None, options=None):
        ...
        self.episode_length=(self.max_episode-1)*self.dt#(self.max_episode-1 - random.uniform(0,30))*self.dt
        return self.state,{}  # reward, done, info can't be included
    
    def reset_pedestrians(self,num_ped, speed_x, speed_y, pos_x, pos_y, dl, direction):
        self.pedestrian = [pedestrian(self.ped_b, self.car_b, self.cross, self.nb_lines,
                                      self.speed_limit, self.dt, simulation=self.simulation,
                                      is_crossing=False, exist=False, number_name=i) for i in range(self.nb_ped)]
        ...
        #dl=self.delta_l_all(cars_pos, cars_speed, cars_line, cars_light)* (self.is_crossing)* ( not self.ped_left)
        
    def reset_pedestrian(self,num_ped, speed_x, speed_y, pos_x, pos_y, dl, direction, cross):
        self.pedestrian[num_ped].reset_ped(speed_x, speed_y, pos_x, pos_y, dl, direction, cross)
    ...
    def reset_distrib(self,state_distrib):
        ...
        for i in range(self.nb_car):
            self.cars[i].reset_car(10., state_distrib[i+input_p*self.nb_ped], 0, i)
        #cars_Sc = [car.Sc for car in self.cars]
        #cars_Vc = [car.Vc for car in self.cars]
        #cars_line = [car.line for car in self.cars]
        #cars_light = [car.light for car in self.cars]
