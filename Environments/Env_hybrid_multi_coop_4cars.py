...
class car_follower:
    ...
    def step(self, obs_leader):
        action=self.follow_action(obs_leader)
        self.transform(action, obs_leader[4])
...
class Crosswalk_hybrid_multi_coop_4cars(gym.Env):
    """Custom Environment that follows gym interface"""
    metadata = {'render.modes': ['human']}
    """
        Crosswalk_hybrid_multi_coop_4cars: creation of the environment
        :param dt: step time
    ...
    def __init__(self, car_b, ped_b, cross_b, nb_car, nb_ped, nb_lines, dt, max_episode, simulation="unif"):
        super(Crosswalk_hybrid_multi_coop_4cars, self).__init__()
        #self.seed()
        self.viewer = False
        self.window = None
    ...
    def step(self, actions):
        ...
        for i in range(self.nb_car):
            self.cars_follow[i].step(self.cars[i].get_data())
    ...
    def reset(self, seed=None, options=None):
        ...
        for i in range(self.nb_car):
            self.cars_follow[i].reset_car(self.speed_limit, self.cars[i].Sc-15., 0, self.cars[i].line)
